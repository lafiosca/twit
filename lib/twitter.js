...
Twitter.prototype.autoTweet = function (status, template, callback) {
  ...
  if (!template.match(/%s/)) {
    return callback('Multi-tweet template must contain %s (status string)');
  }
  ...
  if (status.length <= 140) {
    ...
    while (words.length > 0) {
      word = words.shift();
      if (template.replace(/%s/g, statusString + ' ' + word).length > 140) {
        if (statusString === '') {
          return callback('Failed to break tweet into parts while attempting to fit word: ' + word);
        }
        statuses.push(template.replace(/%s/g, statusString));
        statusString = '';
      }
      ...
      if (statusString === '') {
        ...
        statusString = statusString + ' ' + word;
      }
    }
    statuses.push(template.replace(/%s/g, statusString));
  }
  ...
  if (total > 99) {
    return callback('Tweet broke into too many parts (' + total + ')');
  }

  for (var i = 0; i < total; ++i) {
    statuses[i] = statuses[i].replace(/%n/g, i + 1).replace(/%m/g, total);
  }

  return self.multiTweet(statuses, callback);
}

Twitter.prototype.multiTweet = function(statuses, callback, part, total) {
  ...
  T.post(
    ...
    function(err, data, response) {
      if (err) {
        callback('tweet error on part ' + part + '/' + total + ':' + JSON.stringify(err));
      } else if (statuses.length === 0) {
        callback(null, total);
      } else {
        self.multiTweet(statuses, callback, part + 1, total);
      }
    }
  );
